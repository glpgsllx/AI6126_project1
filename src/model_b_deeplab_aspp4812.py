...
class ASPP4812(nn.Module):
    """ASPP with tighter dilation rates for OS=8."""
    ...
    def __init__(self, in_ch, out_ch):
        ...
        self.conv3 = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, 3, padding=8, dilation=8, bias=False),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True),
        )
...
class LightDeepLabASPP4812(nn.Module):
    """
    Baseline LightDeepLab with only one architectural change:
    ASPP dilation rates [6,12,18] -> [4,8,12].
    """
...
if __name__ == "__main__":
    model = LightDeepLabASPP4812(in_ch=3, num_classes=19, base_ch=32)
    total = count_parameters(model)
    print(f"LightDeepLabASPP4812 parameters: {total:,}")
