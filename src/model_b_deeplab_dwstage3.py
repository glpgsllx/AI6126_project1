...
class ResidualBlockDW(nn.Module):
    """
    Stage3 ablation block:
    replace standard residual convs with depthwise-separable convs.
    """

    def __init__(self, ch, dilation=1):
        super().__init__()
        self.block = nn.Sequential(
            DepthwiseSeparableConv(ch, ch, dilation=dilation),
            DepthwiseSeparableConv(ch, ch, dilation=dilation),
        )
...
class ASPP(nn.Module):
    """Atrous Spatial Pyramid Pooling (same as baseline)."""
...
class LightDeepLabDWStage3(nn.Module):
    """
    Baseline LightDeepLab with a single change area:
    stage3 uses deeper ResidualBlockDW stack.
    """
    ...
    def __init__(self, in_ch=3, num_classes=19, base_ch=32):
        ...
        self.stage3 = nn.Sequential(
            ...
            ResidualBlockDW(b * 4, dilation=1),
            ResidualBlockDW(b * 4, dilation=1),
            ResidualBlockDW(b * 4, dilation=2),
            ResidualBlockDW(b * 4, dilation=2),
            ResidualBlockDW(b * 4, dilation=4),
            ResidualBlockDW(b * 4, dilation=4),
        )  # /8
...
if __name__ == "__main__":
    model = LightDeepLabDWStage3(in_ch=3, num_classes=19, base_ch=32)
    total = count_parameters(model)
    print(f"LightDeepLabDWStage3 parameters: {total:,}")
